...
// 1. Preloader Component (NEW: Pendulum/Expand)
const Preloader = ({ onComplete }: { onComplete: () => void }) => {
    const [index, setIndex] = useState(0);
    const words = ["SYSTEM", "DESIGN", "NETWORK", "INTERFACE"];
    ...
    return (
        ...
        >
            {/* Central Expanding Object */}
            <div className="relative flex flex-col items-center justify-center gap-12 z-10">

                {/* Animated Cross/Pendulum */}
                <motion.div
                    className="relative flex items-center justify-center"
                    animate={{
                        rotate: [0, -15, 15, -45, 90, 0], // Pendulum then snap
                        scale: [0.8, 1.2, 0.9, 1.1, 1, 60], // Random scale then EXPLODE
                    }}
                    transition={{
                        duration: 2.5,
                ...
                >
                    {/* Horizontal Bar */}
                    <div className="absolute w-24 h-2 bg-white/90" />
                    {/* Vertical Bar */}
                    <div className="absolute w-2 h-24 bg-white/90" />

                    {/* The "Mask" that fills screen on explode */}
                    <motion.div
                        className="absolute inset-0 bg-neutral-100 rounded-full"
                        initial={{ opacity: 0 }}
                        animate={{ opacity: [0, 0, 0, 0, 1] }} // Only show at the very end when scaling up
                        transition={{ duration: 2.5, times: [0, 0.5, 0.8, 0.9, 1] }}
                    />
                </motion.div>

                {/* Text Smoothing / Glitch */}
                <div className="h-8 relative overflow-hidden flex flex-col items-center">
                    <AnimatePresence mode="wait">
                        <motion.span
                            key={index}
                            className="text-xs font-mono uppercase tracking-[0.2em] text-neutral-400 absolute"
